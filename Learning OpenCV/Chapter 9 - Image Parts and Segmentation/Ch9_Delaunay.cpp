...
// Date:       2010-01-21
// Coder:      Yishi Guo
// Copy From:  ..\OpenCV2.0\samples\c\delaunay.c
////////////////////////////////////
#include "cv.h"
#include "highgui.h"
#include <stdio.h>
...
void draw_subdiv_edge( IplImage* img, CvSubdiv2DEdge edge, CvScalar color ) {
	...
	if ( org_pt && dst_pt ) {
		...
		cvLine( img, iorg, idst, color, 1, CV_AA, 0 );  // CV_AA: antialiased line
	}
}
...
void run( void ) {
	...
	bkgnd_color = CV_RGB( 255, 255, 255 );  // white
	...
	for ( i = 0; i < max_points; i++ ) {  // Insert the points
...
int main( int argc, char** argv ) {
	...
	max_points = argc >= 2 ? atoi(argv[1]) : 200;
