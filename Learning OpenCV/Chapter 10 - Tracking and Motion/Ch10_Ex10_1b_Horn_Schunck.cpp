...
// Date: 2010-01-27
// Coder: Yishi Guo
// Copy From: F:\CODE\LearningOpenCV_Code\ch10_ex10_1b_Horn_Schunck.cpp
// About: Optical flow by the Horn-Schuck method
////////////////////////////////////////
#include "cv.h"
#include "highgui.h"
#include <math.h>
...
int main( int argc, char** argv ) {
	...
	if ( !(imgA) || !(imgB) ) {
		printf( "One of OpticalFlow0.jpg and/or OpticalFlow1.jpg didn't load\n");
		return -1;
	}
		...
		for ( int x = 0; x < imgC->width; x += step ) {
			if ( px[x] > 1 && py[x] > 1 ) {
				...
				cvLine(
					imgC,
					cvPoint( x, y ),
					cvPoint( (int)x+px[x]/2, (int)y+py[x]/2 ),
					CV_RGB( 255, 0 ,0 ),
					1,
					0
				);
